...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.graduates.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg">+ Add New Graduate</a>
    </div>
                    ...
                    <tr class="border-b border-gray-200 dark:border-gray-600">
                        ...
                        <td class="px-4 py-2">
                            <span class="px-2 py-1 rounded-full text-xs 
                                @if($grad->employment_status == 'employed') bg-green-500 text-white
                                @elseif($grad->employment_status == 'self_employed') bg-blue-500 text-white
                                @else bg-red-500 text-white @endif">
                                {{ str_replace('_', ' ', $grad->employment_status) }}
                            </span>
                        </td>
                        <td class="px-4 py-2">
                            <form method="POST" action="{{ route('admin.graduates.destroy', $grad) }}" onsubmit="return confirm('Delete this graduate?')">
                                @csrf
                                @method('DELETE')
    ...
    @else
        <div class="text-center py-12">
            ...
            <a href="{{ route('admin.graduates.create') }}" class="inline-block mt-4 bg-blue-600 text-white px-4 py-2 rounded-lg">Add Your First Graduate</a>
        </div>
    @endif
</div>
@endsection