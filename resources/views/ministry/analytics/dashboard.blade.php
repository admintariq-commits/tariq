        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="bg-slate-900/95 rounded-[2rem] p-8 shadow-2xl border border-white/10 text-white mb-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <a href="{{ route('ministry.dashboard') }}" class="inline-flex items-center justify-center rounded-full bg-indigo-600 text-white px-5 py-2 text-sm font-semibold hover:bg-indigo-700 transition">Back to Ministry Home</a>
                </div>
            </div>

            <div class="grid gap-6 lg:grid-cols-4 mb-8">
                <a href="{{ route('ministry.analytics.employment-trends') }}" class="group rounded-[1.75rem] border border-slate-800/70 bg-slate-900/95 p-6 shadow-2xl shadow-slate-900/20 hover:border-indigo-400 transition hover:-translate-y-1">
                ...
                <a href="{{ route('ministry.analytics.salary-analysis') }}" class="group rounded-[1.75rem] border border-slate-800/70 bg-slate-900/95 p-6 shadow-2xl shadow-slate-900/20 hover:border-amber-400 transition hover:-translate-y-1">
                ...
                <a href="{{ route('ministry.analytics.skills-gap') }}" class="group rounded-[1.75rem] border border-slate-800/70 bg-slate-900/95 p-6 shadow-2xl shadow-slate-900/20 hover:border-sky-400 transition hover:-translate-y-1">
                ...
                <a href="{{ route('ministry.analytics.profile-completeness') }}" class="group rounded-[1.75rem] border border-slate-800/70 bg-slate-900/95 p-6 shadow-2xl shadow-slate-900/20 hover:border-emerald-400 transition hover:-translate-y-1">
                        ...
                        @foreach($reports as $report)
                            <div class="rounded-3xl bg-slate-800/80 p-4 border border-white/10">
                                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                    <div>
                                        <p class="text-slate-400 text-sm uppercase tracking-[0.16em]">{{ str_replace('_', ' ', ucfirst($report->type)) }}</p>
                                        <h3 class="text-lg font-semibold text-white mt-1">{{ $report->name }}</h3>
                                    </div>
                                    <span class="text-slate-400 text-sm">{{ $report->generated_at->format('M d, Y H:i') }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
